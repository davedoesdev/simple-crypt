...
@param {String|Buffer|Object|undefined} key The key to use for operations using this object.
...
@constructor
*/
function Crypt(key, options) { return undefined; }
...
@param {String|Buffer|Object} key Key to parse. See the `key` parameter of [Crypt](#cryptkey-options).

@return {String|Buffer|Object} Parsed key. You can pass this to [Crypt](#cryptkey-options). If the key looks like a PEM-encoded RSA key then an internal RSA key object is returned. If the key is an object (with `password`, `iterations` and optional `salt` properties) then an object with the following properties is returned:
...
*/
Crypt.parse_key = function (key) { return undefined; };

/**
Get the key being used by this `Crypt` object.

@return {Object|Buffer|String} The key. This could be a `Buffer`, binary-encoded string, internal RSA key object or an object containing a key derived from a password (see [parse_key](#cryptparse_keykey)).
*/

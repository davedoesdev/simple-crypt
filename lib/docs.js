...
***Note: Things have moved on in cryptography since I wrote this library.
While I'm happy to fix bugs in simple-crypt, you're probably better off using
libsodium via [sodium-native](https://github.com/sodium-friends/sodium-native)
or [sodium-plus](https://github.com/paragonie/sodium-plus).**
...
[Istanbul](http://gotwarlost.github.io/istanbul/) results are available [here](http://rawgit.davedoesdev.com/davedoesdev/simple-crypt/master/coverage/lcov-report/index.html).
...
@param {String|Buffer|Object} [key] Optional key to use for operations using this object.
...
@param {Function} [cb] Optional function called with the `Crypt` object. It's passed the following arguments:
...
@return {Crypt} The `Crypt` object. It will have no key until key parsing is complete and `cb` is called.
*/
Crypt.make = function (key, options, cb) { return undefined; };
...
@return {Number} Encryption key size.
*/
Crypt.get_key_size = function () { return undefined; };
...
@return {Number} Initialisation vector size.
*/
Crypt.get_iv_size = function () { return undefined; };
...
@param {String|Buffer|Object} key Key to parse. See the `key` parameter of [Crypt.make](#cryptmakekey-options-cb).

@param {Function} cb Function called with the parsed key. It's passed the following arguments:
...
*/
Crypt.parse_key = function (key, cb) { return undefined; };

/**
Get the key being used by this `Crypt` object.

@return {Object|Buffer|String} The key. This could be a `Buffer`, binary-encoded string, internal RSA key object or an object containing a key derived from a password (see [parse_key](#cryptparse_keykey-cb)).
*/
Crypt.prototype.get_key = function () { return undefined; };
...
@param {Object|Buffer|String} data The data to be encrypted.
...
@param {Buffer|String} [iv] Optional initialisation vector (salt) to use for AES encryption. If not supplied, a random one is created. Length must be equal to [get_iv_size()](#cryptget_iv_size).

@param {Function} cb Function called with the result. It's passed the following arguments:
...
*/
Crypt.prototype.encrypt = function (data, iv, cb) { return undefined; };
...
@param {Object} data A result object received from [encrypt](#cryptprototypeencryptdata-iv-cb). You may have received this from another party, for instance.
...
*/
Crypt.prototype.decrypt = function (data, cb) { return undefined; };
...
@param {Object|Buffer|String} data The data to be signed.
...
@param {Function} cb Function called with the result. It's passed the following arguments:
  ...
  - `{Object} result` Result of signing the data. Typically you would JSON serialize this for transmission, unless you passed `options.base64` as `false` to [Crypt.make](#cryptmakekey-options-cb) in which case `data` and `signature` won't be Base64-encoded. It has the following properties:
    ...
    - `{Number} version` Internal version number for future compatibility checking.
*/
Crypt.prototype.sign = function (data, cb) { return undefined; };
...
@param {Object} data A result object received from [sign](#cryptprototypesigndata-cb). You may have received this from another party, for instance.
...
*/
Crypt.prototype.verify = function (data, cb) { return undefined; };
...
@param {Buffer|String|Object} signing_key Key to use for signing the data.

@param {Buffer|String|Object} encryption_key Key to use for encrypting the data and signature.

@param {Object|Buffer|String} data The data to be signed and encrypted.

@param {Buffer|String} [iv] Optional initialisation vector (salt) to use for encryption. If not supplied, a random one is created.
...
*/
Crypt.sign_encrypt_sign = function (signing_key, encryption_key, data, iv, cb) { return undefined; };
...
@param {Buffer|String|Object} decryption_key Key to use for decrypting the data and signature.

@param {Buffer|String|Object} verifying_key Key to use for verifying the signature.

@param {Object} data A result object received from [sign_encrypt_sign](#cryptsign_encrypt_signsigning_key-encryption_key-data-iv-cb).
...
*/
Crypt.verify_decrypt_verify = function (decryption_key, verifying_key, data, cb) { return undefined; };
...
@param {Object|Buffer|String} data The data to encrypt.
...
@param {Function} [get_key] Optional function to call in order to get the encryption key. You must supply this if you didn't supply a key when creating the `Crypt` object. `get_key` is called with the following arguments:
...
*/
Crypt.prototype.maybe_encrypt = function (encrypt, data, cb, get_key) { return undefined; };

/**
Conditionally decrypt data using [decrypt](#cryptprototypedecryptdata-cb).

@param {Object} data A result object received from [maybe_encrypt](#cryptprototypemaybe_encryptencrypt-data-cb-get_key).
...
*/
Crypt.prototype.maybe_decrypt = function (data, cb, get_key) { return undefined; };

/**
Conditionally sign data using [sign](#cryptprototypesigndata-cb).

@param {Boolean} sign Whether to sign the data.

@param {Object|Buffer|String} data The data to sign.
...
@param {Function} [get_key] Optional function to call in order to get the signing key. You must supply this if you didn't supply a key when creating the `Crypt` object. `get_key` is called with the following arguments:
  ...
  - `{Function} got_key` Function to call with the key. You should call it with the following arguments:
    ...
    - `{Object} [key_data]` Optional metadata for the key. This is included in the result (see above).
*/
Crypt.prototype.maybe_sign = function (sign, data, cb, get_key) { return undefined; };

/**
Conditionally verify data using [verify](#cryptprototypeverifydata-cb).

@param {Object} data A result object received from [maybe_sign](#cryptprototypemaybe_signsign-data-cb-get_key).
...
@param {Function} [get_key] Optional function to call in order to get the verifying key. You must supply this if you didn't supply a key when creating the `Crypt` object. `get_key` is called with the following arguments:
...
*/
Crypt.prototype.maybe_verify = function (data, cb, get_key) { return undefined; };
...
@param {Buffer|String|Object} key Key to use for encryping the data in the stream.

@param {Readable} s The [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to encrypt.

@param {Object} [options] Options for [`frame.encode`](https://github.com/davedoesdev/frame-stream#frameencodeopts).

@param {Function} cb Function called with the encrypted stream. It's passed the following arguments:

  - `{Object} err` If an error occurred then details of the error, otherwise `null`.

  - `{Readable} enc_s` The encrypted data stream. Any encryption errors will be emitted as `error` events on `enc_s`.
*/
Crypt.encrypt_stream = function (key, s, options, cb) { return undefined; };

/**
Decrypt a stream of data.

@param {Buffer|String|Object} key Key to use for decrypting the data in the stream.

@param {Readable} s The encrypted [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to decrypt.

@param {Object} [options] Options for [`frame.decode`](https://github.com/davedoesdev/frame-stream#framedecodeopts).

@param {Function} cb Function called with the decrypted stream. It's passed the following arguments:

  - `{Object} err` If an error occurred then details of the error, otherwise `null`.

  - `{Readable} dec_s` The decrypted data stream. Any decryption errors will be emitted as `error` events on `dec_s`.
*/
Crypt.decrypt_stream = function (key, s, options, cb) { return undefined; };
...
@param {Buffer|String|Object} key Key to use for signing the data in the stream.

@param {Readable} s The [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to sign.

@param {Object} [options] Options for [`frame.encode`](https://github.com/davedoesdev/frame-stream#frameencodeopts).

@param {Function} cb Function called with the signed stream. It's passed the following arguments:

  - `{Object} err` If an error occurred then details of the error, otherwise `null`.

  - `{Readable} sig_s` The signed data stream. Any signing errors will be emitted as `error` events on `sig_s`.
*/
Crypt.sign_stream = function (key, s, options, cb) { return undefined; };

/**
Verify a stream of data.

@param {Buffer|String|Object} key Key to use for verifying the data in the stream.

@param {Readable} s The signed [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to verify.

@param {Object} [options] Options for [`frame.decode`](https://github.com/davedoesdev/frame-stream#framedecodeopts).

@param {Function} cb Function called with the verified stream. It's passed the following arguments:

  - `{Object} err` If an error occurred then details of the error, otherwise `null`.

  - `{Readable} ver_s` The verified data stream. Any verification errors will be emitted as `error` events on `ver_s`.
*/
Crypt.verify_stream = function (key, s, options, cb) { return undefined; };

/**
Sign then encrypt then sign a stream of data.

@param {Buffer|String|Object} signing_key Key to use for signing the data in the stream.

@param {Buffer|String|Object} encryption_key Key to use for encryping the data in the stream.

@param {Readable} s The [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to sign and encrypt.

@param {Object} [options] Options for [`frame.encode`](https://github.com/davedoesdev/frame-stream#frameencodeopts).

@param {Function} cb Function called with the signed and encrypted stream. It's passed the following arguments:
...
*/
Crypt.sign_encrypt_sign_stream = function (signing_key, encryption_key, s, options, cb) { return undefined; };

/**
Verify then decrypt then verify a stream of data.

@param {Buffer|String|Object} decryption_key Key to use for decrypting the data in the stream.

@param {Buffer|String|Object} verifying_key Key to use for verifying the data in the stream.

@param {Readable} s The signed and encrypted [`stream.Readable`](https://nodejs.org/dist/latest-v4.x/docs/api/stream.html#stream_class_stream_readable_1) to verify and decrypt.

@param {Object} [options] Options for [`frame.decode`](https://github.com/davedoesdev/frame-stream#framedecodeopts).

@param {Function} cb Function called with the verified and decrypted stream. It's passed the following arguments:
...
*/
Crypt.verify_decrypt_verify_stream = function (decryption_key, verifying_key, s, options, cb) { return undefined; };

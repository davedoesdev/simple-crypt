    ...
    }, function (err, encrypter)
    {
        ...
        t._transform = function (chunk, encoding, callback)
        {
            ...
            encrypter.encrypt(Buffer.concat(
                    prev_hash ? [Buffer.from([1]), prev_hash, chunk] :
                                [Buffer.from([0]), chunk]),
            function (err, ev)
            {
                if (err)
                ...
                var iv = Buffer.isBuffer(ev.iv) ?
                        ev.iv : Buffer.from(ev.iv, 'binary');
                ths.push(iv);
                to_hash.push(iv);

                var data = Buffer.isBuffer(ev.data) ?
                        ev.data : Buffer.from(ev.data, 'binary');
                ths.push(data);
                to_hash.push(data);
                ...
                buf = Buffer.concat(
                    ev.ekey ? [Buffer.from([1]),
                               Buffer.isBuffer(ev.ekey) ?
                                    ev.ekey : Buffer.from(ev.ekey, 'binary')] :
                              [Buffer.from([0])]);
                ths.push(buf);
                to_hash.push(buf);
        ...
        t._transform = function (chunk, encoding, callback)
        {
            ...
            function (err, sv)
            {
                ...
                var signature = Buffer.isBuffer(sv.signature) ?
                        sv.signature : Buffer.from(sv.signature, 'binary');
                ths.push(signature);
                to_hash.push(signature);

                var data = Buffer.isBuffer(sv.data) ?
                        sv.data : Buffer.from(sv.data, 'binary');
                ths.push(data);
                to_hash.push(data);
...
if (typeof require === 'function')
{
    ...
    Crypt.parse_key = function (key, cb)
    {
        ...
        try
        {
            if (typeof key === 'string')
            {
                ...
                cb2(null, crypto.createSecretKey(Buffer.from(key, 'binary')));
            }
            else if (key && key.password)
            {
                ...
                function (err, derived_key)
                {
                    ...
                    cb2(null,
                    {
                        key: crypto.createSecretKey(derived_key),
                        salt: salt
                    });
                });
            }
    ...
    {
        "use strict";
        return this.options.json ? Buffer.from(JSON.stringify(data)) : data;
    };

    Crypt.prototype.parse = function (data)
    {
        "use strict";
        return this.options.json ? JSON.parse(data.toString()) : data;
    };
    ...
    Crypt.prototype.encrypt = function (data, iv, f)
    {
        ...
        try
        {
            ...
            {
                var buf = crypto.randomBytes(Crypt._AES_128_KEY_SIZE);
                ...
                key = crypto.createSecretKey(buf);
            }
    ...
    Crypt.prototype.decrypt = function (data, f)
    {
        ...
        try
        {
            ...
            {
                key = crypto.createSecretKey(crypto.privateDecrypt(
                {
                    key: this.key.key,
                    padding: crypto.constants.RSA_PKCS1_OAEP_PADDING
                }, this.encoding ? Buffer.from(data.ekey, this.encoding) :
                                   data.ekey));
            }
            ...
            decipher = crypto.createDecipheriv(
                    'AES-128-CBC',
                    key,
                    this.encoding ? Buffer.from(data.iv, this.encoding) :
                                    data.iv);
    ...
    Crypt.prototype.verify = function (data, f)
    {
        ...
        try
        {
            ...
            ddata = this.encoding ? Buffer.from(data.data, this.encoding) :
                                    data.data;
            ...
            else if (!this.key.is_private)
            {
                match = Crypt._buffer_equal(
                        crypto.createHmac('sha256', this.key.key || this.key)
                                .update(ddata).digest(),
                        this.encoding ? Buffer.from(data.signature,
                                                    this.encoding) :
                                        data.signature);
            }
            else

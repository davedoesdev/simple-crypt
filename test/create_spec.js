...
describe('create', function ()
{
    ...
    it('should create with a key and a callback', function (cb)
    {
        ...
        {
            if (err) { return cb(err); }
            expect(process.env.SLOW ? String.fromCharCode.apply(String, crypt.key) : crypt.key.export().toString('binary')).to.equal('some key');
            expect(crypt.options).to.eql(default_opts);
            cb();
        });
    });
    ...
    it('should create with a key, options and a callback', function (cb)
    {
        ...
        {
            if (err) { return err; }
            expect(process.env.SLOW ? String.fromCharCode.apply(String, crypt.key) : crypt.key.export().toString('binary')).to.equal('another key');
            expect(crypt.options).to.eql(opts);
            cb();
        });
    });
